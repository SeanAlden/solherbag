      ...
      <table class="w-full text-left">
        ...
        <tbody class="text-gray-600">
          <tr v-for="trx in transactions" :key="trx.id" class="hover:bg-gray-50 border-gray-50 border-b transition">
            <td class="py-6">
              <span class="block font-mono font-bold text-black text-sm uppercase">{{ trx.order_id }}</span>
            ...
            <td class="py-6">
              <div class="flex -space-x-2">
                <img v-for="detail in trx.details.slice(0, 3)" :key="detail.id" :src="detail.product.image" 
                     class="shadow-sm border-2 border-white rounded-full w-8 h-8 object-cover" :title="detail.product.name" />
                <div v-if="trx.details.length > 3" class="flex justify-center items-center bg-gray-200 shadow-sm border-2 border-white rounded-full w-8 h-8 font-bold text-[10px] text-gray-600">
          ...
          <tr v-if="!isLoading && transactions.length === 0">
            <td colspan="6" class="py-20 font-serif text-gray-400 text-center italic">No transactions found.</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const updateStatus = async (id, newStatus) => {
  try {
    isLoading.value = true;
    const res = await axios.put(`${BASE_URL}/admin/transactions/${id}/status`, { status: newStatus }, axiosConfig);
