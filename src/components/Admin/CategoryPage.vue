  ...
  <div class="relative bg-white shadow-sm p-8 border border-gray-100 rounded-2xl min-h-[400px]">
    <div v-if="isLoading" class="z-20 absolute inset-0 flex justify-center items-center bg-white/60 backdrop-blur-[2px] rounded-2xl transition-all duration-300">
      <div class="flex flex-col items-center">
        <div class="border-4 border-blue-100 border-t-blue-600 rounded-full w-12 h-12 animate-spin"></div>
        <p class="mt-4 font-bold text-blue-600 text-xs uppercase tracking-widest animate-pulse">Fetching Data...</p>
      </div>
    </div>
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="text-gray-600">
          ...
          <tr v-if="!isLoading && categories.length === 0">
            <td colspan="4" class="py-20 text-gray-400 text-center italic">No categories found.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="showModal" class="z-50 fixed inset-0 flex justify-center items-center bg-black/50 backdrop-blur-sm">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div class="flex gap-3 pt-4">
            <button type="button" @click="showModal = false" class="flex-1 py-3 font-bold text-gray-500">Cancel</button>
            <button type="submit" :disabled="isSubmitting" class="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 py-3 rounded-xl font-bold text-white transition">
...
const fetchCategories = async () => {
  ...
  try {
    ...
    // Beri sedikit delay (misal 500ms) agar transisi loading tidak terlalu cepat/flicker
